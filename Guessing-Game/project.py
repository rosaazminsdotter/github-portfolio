...
# Den första funktionen som kallas på och som hämtar in värdena från CSV-filen till highScore-listan samt kallar på meny-funktionen
def start():
    import_scores()
    menu()

# En spel-funktion som kör självaste spelet och laddar upp nya resultat i CSV-filen samt lägger in dem i highScore-listan
def play_game():
    score = 0
    ...
    while True:
        ...
        if guess not in doors:
            ...
            file = open('skattresultat.csv','a', newline='') 
            csv_reader = csv.writer(file, delimiter = ',')
            csv_reader.writerow([name, score])           #Lägger in resultatet i CSV - filen
            highScore.append([name, score])              #Lägger in resultatet i highScore så man slipper imortera CSV-listan under denna körning
            break
        else:
            doors.remove(guess)
            score += 1
...
# En funktion som sorterar och skriver ut poänglistan till skärmen
def print_scores():
    sortedScores = sorted(highScore[1:], key=lambda x: int(x[1]))
    title = highScore[0]
...
def diagram():
    ...
    plt.yticks(range(0, max(frequencyList) + 3))
