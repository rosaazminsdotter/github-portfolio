...
def play_game():
    ...
    while True:
        ...
        if guess not in doors:
            ...
            write_to_csv(name, score) #LÄGGER IN SCORES DIREKT MED APPEND
            break
        else:
            doors.remove(guess)
            score += 1
...
def import_scores():
    try:
        ...
        for rad in csv_reader:
            if len(highScore) > 0:
                add_scores(rad[0],rad[1])
            else:
                highScore.append(['Name', 'Tries'])

        file.close()
        print(highScore) #DEBUG
...
# En funktion som lägger in alla rader i highScore-listan i en sorterad ordning
